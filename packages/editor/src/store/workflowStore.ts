  ...
  loadWorkflow: async (id) => {
    ...
    set({
      ...
      workflowMeta: {
        ...
        webhookSecret: workflow.settings?.webhookSecret ?? '',
        timeout: workflow.settings?.timeout ?? 300000,
        continueOnFail: workflow.settings?.continueOnFail ?? false,
        allowConcurrent: workflow.settings?.allowConcurrent ?? false,
        errorWebhookUrl: workflow.settings?.errorWebhookUrl ?? '',
      },
    });
  },
  ...
  restoreVersion: async (workflowId: string, version: number) => {
    ...
    set({
      ...
      workflowMeta: {
        ...
        webhookSecret: res.workflow.settings?.webhookSecret ?? '',
        timeout: res.workflow.settings?.timeout ?? 300000,
        continueOnFail: res.workflow.settings?.continueOnFail ?? false,
        allowConcurrent: res.workflow.settings?.allowConcurrent ?? false,
        errorWebhookUrl: res.workflow.settings?.errorWebhookUrl ?? '',
      },
    });
  },
